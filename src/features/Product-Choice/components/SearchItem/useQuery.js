...
export const useQuery = () => {
  ...
  const handleFetchItemByArticle = useCallback(
    async (article) => {
      if (!article || article?.length < 4) return;
      const item = await fetchItemByArticle(article);
  ...
  return { query, handleQuery, handleFetchItem, handleFetchItemByArticle };
};
